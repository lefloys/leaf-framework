...
namespace lf {
	...
	struct PrototypeBase {
	  ...
	  protected:
		...
		void load_field(const dict& data, string_view field_name, T& out) {
			...
			if (it == data.end()) {
				throw runtime_exception(format("missing field '{}'", field_name));
			}
		...
		void load_field(const list& data, size_t index, T& out) {
			if (index >= data.size()) {
				throw runtime_exception(format("index {} out of range", index));
			}
	...
	struct PrototypeConnector {
		...
		void resolve(string_view owner_name, string_view field_name, bool required = false, std::optional<id_t> fallback = {}) {
			if (name.empty()) {
				if (required) {
					throw runtime_exception(format("{} missing required field '{}'", owner_name, field_name));
				}
			...
			if (!id) {
				throw runtime_exception(format("{} references missing {} '{}'", owner_name, Proto::type(), name));
			}
		}
	};
